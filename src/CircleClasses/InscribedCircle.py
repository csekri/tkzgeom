...
class InscribedCircle(Circle):
    ...
    def recompute_canvas(self, items, window, width, height):
        ...
        centre = gmath.incentre(A, B, C)
        if centre is None:
            self.set_canvas_centre_xy(*B)
            self.set_canvas_radius(0)
        radius = gmath.inradius(A, B, C)
    ...
    def __str__(self):
        return "Inscribed circle (%s) of %s" % (self.item["id"], self.item["definition"]["A"]+self.item["definition"]["B"]+self.item["definition"]["C"])
