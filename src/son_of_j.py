...
def eucl2tkz(eucl, lbs, tblr=None):
    ...
    return_string = tm.tikzify_duck_new_commands(eucl)
    if return_string != '':
        return_string = '%TIKZDUCKS NEWCOMMANDS\n' + return_string + '\n'

    if eucl["code_before"] != '':
        return_string += '%CUSTOM PRECEEDING CODE\n' + eucl["code_before"] + '\n'
    return_string += tm.tikzify_init(eucl, margin, top, bottom, left, right)
    grid = tm.tikzify_grid(eucl)
    if eucl["axis_x"]["show"] or eucl["axis_y"]["show"]:
        return_string += tm.tikzify_grid_with_any_axis(eucl, grid)
        return_string += tm.tikzify_x_axis(eucl)
        return_string += tm.tikzify_y_axis(eucl)
        return_string += tm.tikzify_axis_clip(eucl)
    else:
        return_string += tm.tikzify_grid_without_axis(eucl, grid)
    return_string += tm.tikzify_all_point_declarations(eucl)
    return_string += tm.tikzify_polygons_and_linestrings(eucl)
    return_string += tm.tikzify_functions(eucl)
    return_string += tm.tikzify_segments(eucl)
    return_string += tm.tikzify_segment_marks(eucl)
    return_string += tm.tikzify_filled_angles(eucl)
    return_string += tm.tikzify_angles(eucl)
    return_string += tm.tikzify_circles(eucl)
    return_string += tm.tikzify_angle_labels(eucl)
    return_string += tm.tikzify_segment_labels(eucl)
    return_string += tm.tikzify_circle_labels(eucl)
    return_string += tm.tikzify_points(eucl)
    return_string += tm.tikzify_point_labels(eucl)
    if eucl["code_after"] != '':
        return_string += '%CUSTOM FOLLOWING CODE\n' + eucl["code_after"] + '\n'
    return_string += tm.tikzify_finalise(eucl)
